...
def run_tests(test_path, coverage=False, verbose=False):
    ...
    if result:
        ...
        # 检查是否没有找到测试文件（pytest 返回 exit code 5 表示没有测试）
        if result.returncode == 5 or "no tests ran" in result.stdout:
            print("提示: 没有找到测试文件，跳过测试")
            return True
...
def main():
    ...
    parser.add_argument(
        "path", nargs="?", default=".", help="测试文件或目录路径 (默认: 当前目录)"
    )
